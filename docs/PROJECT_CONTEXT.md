...
The client is Expo/React Native/TypeScript with Expo Router, Expo Symbols, React Native Web, and an inactive Supabase Auth client boundary for email, Google, and Apple. The server is FastAPI with a locally tested asymmetric Supabase JWT verifier and protected identity smoke route. An owner-controlled Supabase project, social-provider credentials, live JWKS/active-session validation, application persistence, and OpenAI remain unconnected.

Phases 0–2 are implemented and the application is at the Phase 3 authentication checkpoint. A responsive, clickable prototype validates the intended learning loop across web, Android, and iOS code paths; its lesson data, progress, scripted session, and report remain fixed mocks labeled in the UI. The typed authentication state, PKCE client setup, secure native session adapter, email flow, Google OAuth on Android/iOS/web, native Apple login on iOS, Apple OAuth on web, privacy draft, static lesson deep links, root guards, and FastAPI `GET /auth/me` JWT boundary are implemented. The owner approved an account-required current app with any future free experience isolated under `/demo`. The health screen remains the only live frontend-to-server request because no Supabase project or real learner token is configured; `/auth/me` has been verified only with ephemeral local signing material.
