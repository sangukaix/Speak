...
ChatGPT sign-in is enough for local Codex work when the account has access. An OpenAI API key is not required for this repository's current Phase 2 work. Never copy another PC's Codex auth file, paste a token into this repository, or put login credentials in a Codex prompt.
...
The active branch must be `codex/phase-2-ui-foundation`. Do not begin feature work from `main` until the repository documentation says Phase 2 has been merged.
...
In Phase 2, leave these planned integration values blank:
...
They are not required by the current app. Do not create provider projects or keys just to complete setup.
...
Therefore, do not set `EXPO_PUBLIC_API_BASE_URL` for ordinary web/Android emulator setup. Create `frontend/.env` only for an intentional override:

```powershell
Copy-Item frontend/.env.example frontend/.env
```

Replace the file contents with exactly one appropriate line:
...
2. Creates root `.venv` with `py -3.13` when it does not exist.
3. Upgrades pip inside that venv.
4. Installs `backend/requirements-dev.txt`.
...
The frontend's Profile tab links to the developer health screen, which performs the only current real frontend-to-backend request.
